...
export function ProbabilityMarketChart({
  ...
  const summary = selected
    ? `研究方向 ${selected.name}：模型 ${percent(selected.modelProbability)}，$10 可执行 ask ${percent(selected.ask)}；两侧报价独立，不强制互补。`
    : '暂无研究方向；两侧报价独立展示。'
    ...
    <section aria-labelledby="probability-market-title">
      <Card>
        <CardHeader className="border-b">
          ...
          <p className="text-sm text-muted-foreground">实线为模型、虚线为 $10 可执行市场概率；缺失样本保持缺口，不插值。</p>
          <div className="flex flex-wrap gap-2">
            ...
            {overlay === 'gap' ? <Badge variant="destructive"><CircleDashed data-icon="inline-start" aria-hidden="true" />断线不插值</Badge> : null}
            {overlay === 'stale' ? <Badge variant="destructive"><CircleDashed data-icon="inline-start" aria-hidden="true" />STALE · 动作撤销</Badge> : null}
          </div>
        </CardHeader>
          ...
          <div className="grid grid-cols-2 gap-2" aria-label="球员概率选择状态">
            {sides.map((side) => (
              ...
              >
                ...
                <p className="mt-2 font-mono text-lg font-semibold">模型 {percent(side.modelProbability)} <span className="text-muted-foreground">/</span> ask {percent(side.ask)}</p>
              </div>
            ))}
            {sides.length === 0 ? (
          ...
          {trajectory.length === 0 ? (
            <div className="flex min-h-28 items-center gap-3 rounded-lg border border-dashed bg-muted/15 p-4">
              ...
              <p className="text-sm text-muted-foreground">
                轨迹样本积累中：每次决策快照更新会追加一个点；刷新页面后从当前会话重新积累，历史缺口不回填、不插值。
              </p>
            </div>
          ) : (
            ...
            <tbody>
              {trajectory.map((point, index) => (
                <tr key={`${point.time}-${index}`}>
                  ...
                  <td>{point.model === null ? '数据缺口' : percent(point.model)}</td>
                  <td>{point.market === null ? '数据缺口' : percent(point.market)}</td>
                </tr>
              ))}
            </tbody>
          </table>
