...
class ApiTennisProvider:
    ...
    def _validate(self, model: type, payload: Any) -> Any:
        ...
        if not isinstance(parsed, ApiTennisResponse):
            return parsed
        if parsed.success != 1:
            raise AppError(
                "provider_unavailable",
    ...
    async def _map_player(self, external_key: Any, name: str | None) -> Player:
        internal_id = await self._identities.get_or_create(
            "player", PROVIDER_NAME, str(external_key)
        )
        return Player(
            id=internal_id,
            name=(name or "").strip() or "Unknown player",
            country_code=None,
            ranking=None,
        )

    async def _map_match(self, dto: MatchDto) -> Match | None:
        ...
        p1 = await self._map_player(dto.first_player_key, dto.event_first_player)
        p2 = await self._map_player(dto.second_player_key, dto.event_second_player)
        ...
        live_state = self._map_live_state(dto, player_ids, status)
    ...
    def _map_live_state(
        self, dto: MatchDto, player_ids: tuple[str, str], status: MatchStatus
    ) -> LiveMatchState | None:
    ...
    async def _map_matches(self, rows: list[MatchDto]) -> list[Match]:
        mapped = []
        for dto in rows:
    ...
    def _map_points(
        self, dto: MatchDto, match_id: str, player_ids: tuple[str, str]
    ) -> list[PointEvent]:
        observed_at = self._now()
        ...
        for game in dto.pointbypoint:
            ...
            for raw_point in game.points:
                ...
                sequence += 1
                winner_id = self._derive_point_winner(before, after, player_ids)
                quality = None
                ...
                points.append(
                    PointEvent(
                        ...
                        score_after=MatchScore(
                            sets_won=(0, 0), sets=(), points=parsed
                        ),
    ...
    def _map_statistics(
        self, dto: MatchDto, match_id: str, player_keys: tuple[str, str]
    ) -> list[MatchStatistic]:
        observed_at = self._now()
    ...
    def _snapshot_quality(
        self, has_points: bool, has_statistics: bool
    ) -> tuple[DataQuality, ...]:
        observed_at = self._now()
        return (
            DataQuality(
    ...
    async def get_live_matches(self, *, player_id: str | None = None) -> list[Match]:
        ...
        return await self._map_matches(rows)
    ...
    async def get_fixtures(self, *, player_id: str | None = None) -> list[Match]:
        ...
        mapped = await self._map_matches(rows)
        return [match for match in mapped if match.status is MatchStatus.SCHEDULED]
    ...
    async def search_players(self, query: str) -> list[Player]:
        ...
        matches = await self._map_matches([*live_rows, *fixture_rows])
        seen: set[str] = set()
        results: list[Player] = []
    ...
    async def get_match(self, match_id: str) -> Match:
        ...
        if not rows:
            raise AppError("not_found", "Match not found", 404)
        match = await self._map_match(rows[0])
        if match is None:
            raise AppError("not_found", "Match not found", 404)
        return match
    ...
    async def get_match_snapshot(self, match_id: str) -> MatchSnapshot:
        ...
        player_keys = (
            str(dto.first_player_key),
            str(dto.second_player_key),
        )
        points = tuple(self._map_points(dto, match.id, player_ids))
        statistics = tuple(self._map_statistics(dto, match.id, player_keys))
        ...
        return MatchSnapshot(
            ...
            momentum=(),
            quality=self._snapshot_quality(bool(points), bool(statistics)),
    ...
    async def get_recent_results(self, player_id: str, *, limit: int) -> list[Match]:
        ...
        mapped = await self._map_matches(rows)
        finished = [
            match for match in mapped if match.status is MatchStatus.FINISHED
        ]
        finished.sort(
            key=lambda match: (
    ...
    async def get_head_to_head(
        ...
        meetings = await self._map_matches(dto.h2h[:bounded])
        first_recent = await self._map_matches(dto.first_player_results[:bounded])
        second_recent = await self._map_matches(dto.second_player_results[:bounded])
        return HeadToHead(
