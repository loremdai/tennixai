...
**当前动作：** P3 SOTA 研究方向、首版模型覆盖标记、market-to-match 运行时组合、单场 paper opportunity 和“独立估值而非延迟套利”原则已获用户批准。下一步冻结价值分歧如何转化为 `BUY / WAIT / NO BET` 及动态入场价，再继续讨论实时数据流和页面信息架构；全部设计获批前不写实现代码。
...
- 一旦开仓，同场不加仓、不反向换边、不提前退出，持有至市场结算；这样一场比赛只贡献一个明确且可审计的交易结果，不会把高度相关的连续信号虚增成多次机会。
- 若整场没有出现合格 `BUY`，结果为 `NO BET`，不生成模拟持仓；所有合格与不合格观察仍可进入评估轨迹。
- 动态加仓、反向、止盈止损和提前卖出策略统一留到 P4，在拥有足够 observation 数据后单独回测，不能混入 P3 首版模型质量评估。
...
继续 T55 的单问题设计讨论；下一项冻结价值分歧的产品状态：是否在 `BUY / NO BET` 之间增加带动态目标买入价的 `WAIT`，并据此定义首笔 paper position 的触发规则。随后再讨论实时流与页面。全部设计经用户批准后写入 P3 设计规格；规格获批前不得编写实施计划、修改 v0 原型或实现 P3 功能。
