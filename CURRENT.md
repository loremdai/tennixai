...
- **已确认的数据事实：** 真实页 Jannik Sinner 的英文/中文名、意大利国籍与国旗、生日/年龄、ATP #1、11,500 分、排名变化及北京时间更新时间均正确。无当前单打比赛符合 T101 过滤后的数据；2026 年历史记录共 47 场、3 页。API-Tennis player stats 未返回 2026 赛季汇总；不能把不保证完整的逐场记录聚合成官方胜负/场地统计，因此仍诚实显示“暂无”。少数已结束比赛只含总盘数和部分局分，不能补造空盘分数。供应商轮次前缀是展示冗余；历史对手则可按内部 ID 从已有目录恢复完整姓名、中文名和国籍。
- **已完成修复：** 比赛摘要与逐场赛果在缺少正式赛季汇总时明确分开；历史比分保留已知总盘数，并将可用局分标为“部分局分”，不再出现空盘占位符；赛事轮次移除重复赛事/巡回赛前缀；清除面向开发者的“档案/不提供场地筛选”提示。后端通过一次批量目录读取，为当前比赛和当前历史结果页补齐 canonical 对手姓名、中文名及国籍，不增加供应商请求。
- **验证节点：** 浏览器已检查真实 profile、结果筛选/重置/分页和桌面/390px 手机视口；当前运行中的旧 API 仍会把历史对手显示为缩写。2026-09-25 真实只读核验：球员赛果接口返回 47 场，但首场对手在比赛记录中为 `A. Zverev`、中文名/国籍为空；用同一个内部球员 ID 查询其资料接口，则返回完整英文名、中文名和 `DE`。这证明资料已在本地目录中，差异是旧 API 进程未加载 T102 的批量目录补全代码，不是供应商缺数据。T102 新代码的目录补全尚待真实页面确认。筛选从“胜”重置回 47 场全部结果；分页正确切至 21–40/44 后返回。Backend 全部非 integration/live 测试 `1336 passed`，球员专项 `54 passed`；frontend Vitest `36 files / 488 passed`、TypeScript、改动文件 Ruff 和 `git diff --check` 通过。
- **代码检查点：** `6347135` 已推送到 `origin/main`；T102 仍在进行，真实 API 页面确认依赖用户批准的服务重启。
- **待确认：** 项目启动器不支持仅重启 FastAPI；使后端改动进入页面需整套 `down/up`（会短暂停止 runtime/API/frontend 和项目自有 Postgres/Redis 容器，保留数据卷）。已向用户说明准确影响，等待是否批准整套重启。未执行 `init`，未读取/输出根 `.env`。
...
| 日期 | 提交 | 事实 |
|---|---|---|
| 2026-09-25 | `6347135` | T102 球员页字段修复检查点：规范化历史比分/轮次、移除开发者提示、以目录批量补齐比赛对手身份；后端 1336、前端 488 项通过。等待整套本地服务重启批准，尚未关闭任务。 |
