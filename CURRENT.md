...
**当前动作：** P3 SOTA 研究方向、首版模型覆盖标记、market-to-match 运行时组合、“独立估值而非延迟套利”原则，以及一次入场/最多一次规则化退出的 paper 生命周期已获用户批准。规则化退出的论文、行为金融和 Polymarket 执行约束调研已完成，但默认退出策略尚未批准；下一步先决定是否把“期望值卖出”和“可选锁定利润”拆开，再继续讨论行动状态、实时数据流和页面信息架构。
...
- 尚未批准或实施：候选模型 champion、绝对阈值、默认退出目标函数/策略、训练数据许可方案、P3 数据/服务契约、页面改版和任何交易能力。
...
- 一旦开仓，同场不加仓、不反向换边、不重新入场；允许在市场价格向独立模型估值收敛、剩余 edge 消失或反转时按预先冻结的规则卖出一次，否则持有至市场结算。这样一场比赛仍只贡献一个明确且可审计的 position lifecycle，不会把高度相关的连续信号虚增成多次机会。
- 对提前退出的持仓，同时保存实际退出结果和“若继续持有到结算”的反事实结果，以检验退出时机是否真正改善收益；反事实不计作第二笔交易。
- 若整场没有出现合格 `BUY`，结果为 `NO BET`，不生成模拟持仓；所有合格与不合格观察仍可进入评估轨迹。
- 动态加仓、反向、重新入场、任意止盈止损和多次买卖统一留到 P4；P3 只建立最小的一次规则化价值收敛退出能力，具体阈值仍须先冻结并用 observation 数据评估。
...
继续 T55 的单问题设计讨论；下一项决定是否采用“期望值建议与风险锁利分轨”：默认 `SELL` 只在延迟/深度/费用后的净卖出价值超过稳健继续持有价值时触发；市场已收敛但只相当于模型价值时显示可选 `LOCK PROFIT`，不宣称提高期望值。paper 同时保留 HODL、EV-exit 和 convergence-lock 反事实轨道，固定止盈只作诊断 benchmark。随后再讨论 `BUY / WAIT / NO BET`、实时流与页面。全部设计经用户批准后写入 P3 设计规格；规格获批前不得编写实施计划、修改 v0 原型或实现 P3 功能。
