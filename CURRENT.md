...
**当前动作：** P3 SOTA 研究方向、首版模型覆盖标记、market-to-match 运行时组合、“独立估值而非延迟套利”原则，以及一次入场/最多一次价值收敛退出的 paper 生命周期已获用户批准。下一步冻结规则化退出触发语义，再继续讨论行动状态、实时数据流和页面信息架构；全部设计获批前不写实现代码。
...
继续 T55 的单问题设计讨论；下一项冻结规则化退出触发：应随当前模型估值判断“剩余持有 edge 已消失/反转”，而不是使用固定盈利百分比或事后挑选最佳卖点。随后再讨论 `BUY / WAIT / NO BET`、实时流与页面。全部设计经用户批准后写入 P3 设计规格；规格获批前不得编写实施计划、修改 v0 原型或实现 P3 功能。
