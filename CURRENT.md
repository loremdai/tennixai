...
- **验证节点：** 浏览器已检查真实 profile、结果筛选/重置/分页和桌面/390px 手机视口；当前运行中的旧 API 仍会把历史对手显示为缩写，因此新后端目录补全尚待真实页面确认。筛选从“胜”重置回 47 场全部结果；分页正确切至 21–40/44 后返回。Backend 全部非 integration/live 测试 `1336 passed`，球员专项 `54 passed`；frontend Vitest `36 files / 488 passed`、TypeScript、改动文件 Ruff 和 `git diff --check` 通过。
