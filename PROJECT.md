...
承担单场 Investigation：比分、状态、发球方、上下文问答，以及 P2/P3 的统计、PBP、走势、市场和决策支持。P3 采用“全宽决策条”布局：既有比赛 Hero 保持首位，其后用跨越主栏和侧栏的 `DecisionSummary` 先回答模型观点、当前动作、可执行市场概率、保守净 edge 与 freshness；下方继续沿用比赛事实主栏 + 粘性助手/关键事实侧栏。详细概率—市场轨迹、结构化依据和本场 paper lifecycle 放入主栏，旧侧栏市场占位不再保留，避免重复。页面必须携带内部 `match_id`，用户无需重复比赛上下文。
