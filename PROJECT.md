...
**P4.3 设计：** [市场数据可信度与覆盖](./docs/superpowers/specs/2026-09-22-tennixai-p4-market-data-truthfulness-and-coverage-design.md)（T83 已设计冻结，2026-09-22 用户书面确认；T84–T89 已按该规格全部交付并关闭，2026-09-23，详细计划见 `docs/superpowers/plans/`）。
...
- P4.3 首次交付双通道：`/markets` 通过有界 REST batch snapshot 获得真实报价，严格映射且合格的主巡单打由实时 WebSocket 驱动模型/decision/paper。T91 在保留此边界的前提下，为全部展示 listing 新增独立只读广目录 WebSocket 与 REST 校准；它不复用严格决策流，也不触发 PredictionService、DecisionWorker 或 PaperTradingService。`market_match_links` 是 market→match 的查询真相，不能由空的 `markets.match_id` 代替。报价状态明确区分 realtime、snapshot、partial、no liquidity、unavailable、stale 和 limited；每行的 `model_availability` 与 `decision_action` 只来自真实模型/决策记录。「机会」在模型未晋升时明确说明不会产生 `BUY/WAIT`（真实本地端点曾返回 `ELIGIBLE_UNPROMOTED`、零 `BUY/WAIT`）。该优化不训练/晋升模型、不改变决策门、paper lifecycle 或自动交易边界。
- T91 已确认的展示边界：所有活跃的网球单场胜者市场（包括暂时无法匹配球员身份的场次和双打）都展示供应商提供的真实 outcome 名称及真实报价；展示不要求构造球员 ID。实现方向为完整 Gamma keyset 目录 + 单条只读广目录 WebSocket，REST 用于启动/重连校准，报价变更通过独立 SSE 提醒页面。无法严格匹配到合格单打比赛的市场不得进入模型、机会或 paper。无挂单时如实显示无流动性，不以模型概率或估算值补价；模型未晋升时机会页仍应诚实为空。
- P4.4 网络环境边界：项目自建 HTTPX 客户端使用 `trust_env=False`，WebSocket 明确禁用隐式代理；系统 `HTTP(S)_PROXY` / `NO_PROXY` 不会意外影响 API、runtime、CLI、市场或 LLM 连接。未来若需企业代理，必须增加显式项目配置。
...
**面向消费者的界面原则：** 页面先回答普通网球用户最关心的问题；使用自然中文和常见网球术语，不把 P1–P4、供应商、内部 ID、模型版本或测试状态当作产品文案。缺失、延迟和暂不可用的信息要如实说明，不用猜测值填空；球员姓名在所有结构化界面中统一以英文为主、中文为辅，中文名缺失时只显示英文，不补占位字；AI 自然语言回答保持自然表达，不强行改写。球员照片只使用供应商明确提供并按内部 `player_id` 可靠关联的图片，在现有球员目录中复用 `image_url` 缓存；无图或身份不确定时使用中性头像，不用姓名首字母或猜测照片；更深的技术依据按需展开。
