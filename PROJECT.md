...
- P4 用于基于真实使用和 paper-trading 证据统一打磨 P1–P3，包括事实查询体验、实时质量、模型校准、决策阈值、仓位管理、性能与产品细节；先搭完整框架，再做细致优化。P4.0 已冻结完整本地真实运行设计及其 T73–T80 实施路线：单一 `init/up/status/down` 入口、独立 `tennix_live_local` 数据库、runtime 独占两条上游 WebSocket、低频赛程/排名同步、显式 stale/gap 与 `paper` 边界；P4.1 只按该计划逐任务实施，不授权自动下单或模型晋升。
