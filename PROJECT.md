...
- P4.3 已交付双通道：`/markets` 的全部 canonical market 以有界公开 batch snapshot 获得真实、带时间的展示报价；严格映射且合格的主巡单打仍由 WebSocket 驱动模型/decision/paper。`market_match_links` 是 market→match 的查询真相，不能由空的 `markets.match_id` 代替。报价状态明确区分 realtime、snapshot、partial、no liquidity、unavailable、stale 和 limited，且每行显式给出 `model_availability` 与只来自真实决策记录的 `decision_action`；「机会」在模型未晋升时明确说明不会产生 `BUY/WAIT`（真实本地端点返回 `ELIGIBLE_UNPROMOTED`，零 `BUY/WAIT`）。展示车道只读、无凭据、零 LLM，不触发 PredictionService、DecisionWorker、PaperTradingService 或 WebSocket 订阅；该优化不训练/晋升模型、不改变决策门、paper lifecycle 或自动交易边界。
