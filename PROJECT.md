...
**面向消费者的界面原则：** 页面先回答普通网球用户最关心的问题；使用自然中文和常见网球术语，不把 P1–P4、供应商、内部 ID、模型版本或测试状态当作产品文案。缺失、延迟和暂不可用的信息要如实说明，不用猜测值填空；球员照片只使用供应商明确提供并可靠关联到内部球员 ID 的图片，无图或身份不确定时使用中性头像，不用姓名首字母或猜测照片；更深的技术依据按需展开。
