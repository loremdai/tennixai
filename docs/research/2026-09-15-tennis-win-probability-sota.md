...
**适用范围：** 赛前与赛中、职业网球单打、单场比赛胜者市场、固定 `$10` paper stake、买入后持有至市场结算
...
- 每个合格机会固定 `$10`，首版买入后持有到结算；不复利、不动态加仓、不止盈止损。
- 保存 prediction/model/calibration/data/market/rule 版本，以及信号时订单簿快照、预期成交、费用和弃权原因。
...
- 不在 P3 混入动态退出、Kelly 仓位、自动下单或 latency-arbitrage 承诺；这些保留给有证据后的 P4/独立阶段。
