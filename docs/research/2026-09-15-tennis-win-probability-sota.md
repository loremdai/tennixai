...
## 19. T55 后续批准：Match Page 全宽决策条

**批准日期：** 2026-09-16

对照现有 Match Page 的 Hero、比赛事实主栏和粘性助手侧栏后，用户在三种布局中选择方案 B：

1. 既有 Hero 保持在顶部，继续作为对阵、比分、发球方和比赛状态的首要事实源。
2. Hero 下方加入跨越两栏的全宽 `DecisionSummary`，先展示模型观点、当前动作、固定 `$10` 的可执行市场概率、保守净 edge 和 freshness；它不能取代比分，也不展开完整分析。
3. 下方保留现有两栏骨架。详细概率—市场轨迹、结构化依据和 paper position lifecycle 进入主栏，与比分、统计、PBP、近期控制共同构成单场调查流；本场助手和关键事实继续位于粘性侧栏。
4. 现有侧栏 `MarketCard` 占位在实现 P3 时移除，避免与全宽摘要形成两个决策真源。

该布局把“现在如何判断”提升到首屏，同时保留赛况与判断依据的同屏关系。`DecisionSummary` 的具体双边/单边组织、移动端顺序和状态矩阵仍需后续批准。

## 20. T55 后续批准：`DecisionSummary` 双边对照

**批准日期：** 2026-09-16

用户在“单边动作优先、双边对照 + 动作结论、轨迹优先”三种摘要组织中选择双边对照：

- 摘要顶部先给模型观点和唯一当前动作，避免用户从多项指标自行猜测系统结论。
- 主体同时展示两位球员，并为每一侧独立列出模型概率、固定 `$10` 按订单簿深度计算的可执行平均买入价和扣除不确定性、费用与滑点后的保守净 edge。当前方向可以高亮，但另一侧始终可核对。
- 两侧 Polymarket ask/depth 是独立可执行成本，可能因 spread、流动性和费用不互补；UI 不把两边价格强制归一为 100%，也不以 midpoint 取代真实买入成本。
- 摘要底部显示硬门状态、适用时的动态最高可接受平均价、两条输入流的 freshness 和 decision version。概率—市场轨迹留在下方详细模块，避免摘要与图表重复。

该结构兼顾结论速度与透明度，并可复用到 `BUY / WAIT / NO BET` 及持仓后动作；各状态的具体字段增减和视觉变化仍需单独批准。

## 21. T55 后续批准：成交后原地切换为 position 管理

**批准日期：** 2026-09-16

用户在“单一摘要原地切换、机会与持仓上下叠放、摘要内部标签”中选择原地切换：
