...
- `Final: Ruling: provider omission means stale, not deletion — API-Tennis's documented sample permits empty statistics, but does not define empty as a metric withdrawal; retaining values with an explicit stale label and original time is the safer interpretation — cost if wrong: an intentionally withdrawn metric may remain visible as stale through or after match completion.`
- `Final: Ruling: defer PostgreSQL/Redis round-trip verification — the deterministic reducer/publisher path is covered, while the available infrastructure suite targets the shared local database/runtime and T93 changes no schema — cost if wrong: stale availability or refreshed timestamps could fail to survive persistence/reload.`
- `Final: Ruling: defer browser visual/Playwright verification — the shared Next runtime uses `.next` and `frontend/next-env.d.ts` is user-owned; component rendering and its focused UI tests pass without touching that runtime — cost if wrong: a narrow-viewport layout issue may remain undiscovered.`
- `Final: Ruling: scope completion evidence to freshly rerun deterministic gates — final-state backend `1241 passed/127 deselected`, frontend `416 passed`, and TypeScript passed; live-provider, infrastructure, and browser E2E lanes were not run — cost if wrong: an environment-specific integration defect is outside this evidence.`
- `Final: Ruling: do not change quality semantics for complete snapshots here — the review did not establish a reproducible full-snapshot defect, and this task targets sparse updates; include it in the broader field audit — cost if wrong: a separate quality inconsistency may persist until that audit.`
