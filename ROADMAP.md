...
**总体状态：** `in_progress`（P1 基线已完成；T18 问答展示硬化中；P2 保持 planned）

**当前里程碑：** P1 — 比赛信息查询助手（T18 硬化中；P2 尚未启动）

**当前阶段：** P1.6 — Acceptance and hardening（T18 `in_progress`）
...
| 里程碑 | 状态 | 目标 | 进入/完成条件 |
|---|---|---|---|
| P1 — Match Information Assistant | `in_progress` | 跑通真实结构化比赛查询、卡片、Match Page 与上下文 Chat | T17 已完成（`69c8238`）；T18 正修复问答 Markdown 展示，完成后重新关闭 P1 |
...
| P1.6 — Acceptance and hardening | `in_progress` | 验收集、真实服务 opt-in 测试、Playwright、真实运行时边界与修复 | T16/T17 已完成；T18 修复问答 Markdown 原文显示并补充浏览器证据 |
...
| T18 | P1.6 | Render Markdown in Conversational Answers | `in_progress` | — | 截图已确认问答 prose 原样显示 `**` 与 `-`；正在定位渲染责任组件，完成门为回归测试、typecheck/build、Playwright 和真实浏览器展示验收 |
