...
- P3 页面首轮：采用 Home → `/markets` → Match 三层信息架构。Home 仅给少量高价值机会与开放持仓摘要；`/markets` 是跨比赛扫描与 paper ledger 入口，首版包含当前机会、即将开始、开放持仓、近期结算四类内容；Match Page 承担单场模型—市场对照、动作、轨迹、解释与 position lifecycle。所有卡片用内部 `match_id` 进入 Match Page；`/markets` 不成为自动交易终端。
