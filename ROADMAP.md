...
| T39 | P2.5 | Design and Implement Scope-Aware Multi-Tool Chat Orchestration | `in_progress` | `9b88c0e`（spec） | 书面规格已完成并推送；待用户评审后按 TDD 实现 scope/phase/依赖声明、无依赖并行、有依赖串行、非法调用重规划和部分失败降级，并完成全量回归与真实浏览器复核 |
