...
**当前阶段：** P1.6 — Acceptance and hardening（已完成）
...
| 里程碑 | 状态 | 目标 | 进入/完成条件 |
|---|---|---|---|
| P1 — Match Information Assistant | `done` | 跑通真实结构化比赛查询、卡片、Match Page 与上下文 Chat | 已完成（`5dcaa6a`）：T15 的 fake/前端/视觉基线加上 T16 真实 provider、真实 LLM、真实组合 chat 和真实浏览器门全部通过，证据见任务登记表 |
...
| P1.6 — Acceptance and hardening | `done` | 验收集、真实服务 opt-in 测试、Playwright、runbook、真实运行时边界 | T16（`5dcaa6a`）完成真实数据摘要上限、45 秒可配置总时限、球员重复/组合名消歧；确定性、真实 provider/LLM/组合和浏览器门均有实际证据 |
