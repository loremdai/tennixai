...
- P3 决策首轮：覆盖赛前与赛中，只做单场比赛胜者市场；每场最多一次固定 `$10` paper 入场和一次规则化退出。市场价格向独立模型估值收敛、剩余 edge 消失或反转时可退出，否则持有至结算；同场不加仓、不换边、不重新入场，并保存持有到结算的反事实结果。
