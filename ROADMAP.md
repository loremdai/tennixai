...
**总体状态：** `in_progress`（P1 核心实现完成，T16 真实运行时硬化进行中；P2 保持 planned）

**当前里程碑：** P1 — 比赛信息查询助手（真实运行时硬化）

**当前阶段：** P1.6 — Acceptance and hardening（T16）
...
| 里程碑 | 状态 | 目标 | 进入/完成条件 |
|---|---|---|---|
| P1 — Match Information Assistant | `in_progress` | 跑通真实结构化比赛查询、卡片、Match Page 与上下文 Chat | 核心实现已完成（`98075ef`）；真实 provider 已通过，但真实 provider + LLM 全局 live 查询暴露上下文过大/无总时限问题，T16 修复并复验后关闭 |
...
| P1.6 — Acceptance and hardening | `in_progress` | 验收集、真实服务 opt-in 测试、Playwright、runbook、真实运行时边界 | T15（`98075ef`）完成 fake/真实 LLM 门；T16 处理真实 LiveTennisAPI 数据进入 LLM 时的上下文上限、总时限和球员消歧，计划见 [T16 计划](./docs/superpowers/plans/2026-09-08-tennixai-t16-real-runtime-hardening.md) |
...
| T16 | P1.6 | Harden the Real Live Runtime | `in_progress` | `n/a` | 领取提交 `93601bd`；已复现：真实 provider 50 场 live 数据可读，真实 LLM 单项门通过，但真实组合 chat 在完整 tool payload 上无 `done`；Djokovic 查询因重复实名/组合名触发 `ambiguous_player`。实施与验收见 [T16 计划](./docs/superpowers/plans/2026-09-08-tennixai-t16-real-runtime-hardening.md) |
